...
var (
	...
	// ErrInvalidTenant: the tenant field must be a UUID, or (in a pattern only)
	// the PlatformTenant token.
...
// errTokenInConcreteCRN rejects the PlatformTenant token outside a pattern. It is
...
// other bad tenant.
var errTokenInConcreteCRN = errors.New("crn: the tenant token is only valid in a pattern")
